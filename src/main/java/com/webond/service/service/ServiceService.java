...
public class ServiceService {
	...
	@Transactional(readOnly = true)
	public List<ServiceVO> searchActiveServices(String keyword, Integer serviceTypeId, String serviceCity,
			String serviceDistrict, Integer minRate, Integer maxRate) {

		String normalizedKeyword = normalizeOptionalText(keyword);

		String normalizedCity = normalizeOptionalText(serviceCity);

		String normalizedDistrict = normalizeOptionalText(serviceDistrict);

		if (serviceTypeId != null && serviceTypeId <= 0) {

			throw new IllegalArgumentException("服務類型編號不正確");
		}

		if (minRate != null && minRate < 0) {

			throw new IllegalArgumentException("最低金額不可小於 0");
		}

		if (maxRate != null && maxRate < 0) {

			throw new IllegalArgumentException("最高金額不可小於 0");
		}

		if (minRate != null && maxRate != null && minRate > maxRate) {

			throw new IllegalArgumentException("最低金額不可大於最高金額");
		}

		return serviceRepository.searchActiveServices(normalizedKeyword == null ? "" : normalizedKeyword,

				serviceTypeId,

				normalizedCity == null ? "" : normalizedCity,

				normalizedDistrict == null ? "" : normalizedDistrict,
	...
	public ServiceVO addBySeller(Integer loginMemberId, Integer serviceTypeId, String serviceName, String description,
			Integer hourlyRate, byte[] serviceImage, String serviceImageType, String serviceCity,
			String serviceDistrict, String serviceLocation) {

		if (loginMemberId == null) {
			throw new IllegalArgumentException("請先登入");
		}

		validateMemberServiceRequest(serviceTypeId, serviceName, description, hourlyRate, serviceCity, serviceDistrict,
				serviceLocation);
		...
		serviceVO.setServiceImageType(normalizeImageType(serviceImage, serviceImageType));

		// 服務地區
		serviceVO.setServiceCity(serviceCity.trim());
		serviceVO.setServiceDistrict(serviceDistrict.trim());
		serviceVO.setServiceLocation(normalizeOptionalText(serviceLocation));
	...
	/*
	 * 沒有上傳圖片時也可以使用此方法。 主要是避免舊 Controller 尚未傳入圖片參數時編譯失敗。
	 */
	public ServiceVO addBySeller(Integer loginMemberId, Integer serviceTypeId, String serviceName, String description,
			Integer hourlyRate, String serviceCity, String serviceDistrict, String serviceLocation) {

		return addBySeller(loginMemberId, serviceTypeId, serviceName, description, hourlyRate, null, null, serviceCity,
				serviceDistrict, serviceLocation);
	}
	...
	/*
	 * 修改服務基本資料與地區。 此方法不修改圖片，原本圖片會保留。
	 */
	public void updateBySeller(Integer serviceId, Integer loginMemberId, Integer serviceTypeId, String serviceName,
			String description, Integer hourlyRate, String serviceCity, String serviceDistrict,
	 ...
	 * 修改服務基本資料、地區及圖片。
	 *
	 * replaceImage： true = 使用本次傳入的圖片取代舊圖片 false = 保留原本圖片
	 */
	public void updateBySeller(Integer serviceId, Integer loginMemberId, Integer serviceTypeId, String serviceName,
			String description, Integer hourlyRate, byte[] serviceImage, String serviceImageType, boolean replaceImage,
			String serviceCity, String serviceDistrict, String serviceLocation) {
	...
	public void deleteBySeller(Integer serviceId, Integer loginMemberId) {
		...
		/*
		 * 沒有任何訂單時： 先刪除此服務底下的時段，再刪除服務， 避免 SERVICE_SLOT 外鍵擋住。
		 */
	...
	private ServiceVO getOwnServiceOrThrow(Integer serviceId, Integer loginMemberId) {
		...
		if (serviceId == null || serviceId <= 0) {
			throw new IllegalArgumentException("服務編號不正確");
		}
	...
	private void validateServiceCanBeModified(ServiceVO serviceVO) {
	...
	private void validateMemberServiceRequest(Integer serviceTypeId, String serviceName, String description,
		...
		if (serviceCity == null || serviceCity.trim().isEmpty()) {

			throw new IllegalArgumentException("請選擇服務縣市");
		}

		if (serviceCity.trim().length() > 20) {
			throw new IllegalArgumentException("服務縣市不可超過 20 個字");
		}

		if (serviceDistrict == null || serviceDistrict.trim().isEmpty()) {

			throw new IllegalArgumentException("請選擇服務行政區");
		}

		if (serviceDistrict.trim().length() > 20) {
			throw new IllegalArgumentException("服務行政區不可超過 20 個字");
		}

		if (serviceLocation == null || serviceLocation.trim().isEmpty()) {

			throw new IllegalArgumentException("請填寫服務地點說明");
		}

		if (serviceLocation.trim().length() > 255) {
			throw new IllegalArgumentException("服務地點說明不可超過 255 個字");
		}
	}
	...
	private String normalizeImageType(byte[] serviceImage, String serviceImageType) {

		if (serviceImage == null || serviceImage.length == 0) {
			return null;
		}

		if (serviceImageType == null || serviceImageType.trim().isEmpty()) {
	...
	// 停權or註銷下架
	public void disableAllActiveServicesByMemberId(Integer memberId) {

		if (memberId == null || memberId <= 0) {
			throw new IllegalArgumentException("會員編號不正確");
		}

		serviceRepository.disableAllActiveServicesByMemberId(memberId);
	}
}