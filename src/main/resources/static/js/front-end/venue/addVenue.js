...
function previewImages() {
    ...
    Array.from(input.files).forEach((file, index) => {
        ...
        reader.onload = function(e) {
            ...
            wrapper.onclick = () => setCover(index, wrapper);

            const img = document.createElement('img');
            img.src = e.target.result;
            img.style.cssText = 'width: 100px; height: 80px; object-fit: cover; border-radius: 6px; border: 2px solid ' + (index === 0 ? '#1d9e75' : '#e0e0e0') + ';';
            img.dataset.index = index;

            const label = document.createElement('div');
            label.textContent = index === 0 ? '封面' : '';
...
function setCover(index, wrapper) {
    ...
    document.querySelectorAll('#previewArea .cover-label').forEach(label => {
        label.textContent = '';
    });

    wrapper.querySelector('img').style.borderColor = '#1d9e75';
    wrapper.querySelector('.cover-label').textContent = '封面';
}
